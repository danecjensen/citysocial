...
- The fresh clone can boot on a DETACHED HEAD that is AHEAD of the local `master` ref (seen: b575592 vs stale 7423bdb — merged PRs missing from local master). `git checkout -B <branch> master` then silently bases on the stale ref and reverts files. ALWAYS `git fetch origin master` in Phase 0 and branch off `origin/master`, never the bare local `master`. Re-run `bin/verify` after re-basing to confirm you built on the true master.
...
## 2026-08-07 11:59 — F-022
Outcome: shipped
PR: https://github.com/danecjensen/citysocial/pull/29
Changed: app/javascript/controllers/share_controller.js, components/platform_core/app/public/platform_core/ui/share_component.rb, components/platform_core/app/public/platform_core/ui/button_component.rb, components/events/app/views/events/events/show.html.erb, app/views/design/show.html.erb, spec/components/platform_core/ui/share_component_spec.rb, spec/components/platform_core/ui/button_component_spec.rb, spec/requests/events_spec.rb, docs/roadmap.md, routines/backlog.json, routines/progress.md
Notes: Capability lane. Shipped a reusable progressive-enhancement share primitive with a permanent canonical copy fallback, conditional browser-native sharing, accessible outcome status, and Events as the first public consumer. The engagement loop is discover an Austin event -> share its CitySocial page into an existing chat -> friends open it and coordinate. R-009 remains on research-only PR #28, so research.md was not edited; DanesIdeas Inbox was empty. Repaired the malformed default backlog using the canonical F-016 reconciliation and reserved F-021 from open PR #26. Verification incomplete: Packwerk, RuboCop, Zeitwerk, Rails rendering/ERB, JavaScript syntax and behavior smokes, Ruby syntax, Tailwind, JSON, boundary, and diff checks passed; managed PostgreSQL denial blocked all 39 RSpec files before examples, and screenshot tooling was unavailable because DNS/package and local-file browser policies both blocked capture.
...
## 2026-08-11 17:40 — F-038
Outcome: shipped
PR: https://github.com/danecjensen/citysocial/pull/47
Changed: components/messaging/app/views/messaging/conversations/index.html.erb, spec/requests/messaging_spec.rb, routines/backlog.json, routines/progress.md
North Star: Strengthens re-engagement by letting a resident recognize which listing/post a thread is about at a glance, so utilitarian conversations resume without hunting.
Notes: Same session, back-to-back run. origin/master unchanged (b789b02, verified green this session); PR #46 (F-037) NOT merged, so master's backlog is a run behind my own in-flight work. Circuit breakers: 5 open claude/* PRs on arrival (<6) — this PR makes 6, so the NEXT run trips the queue breaker. 1a: DanesIdeas Inbox empty. Reconciliation: rather than re-run a fresh 1b off stale master (which would re-discover the same gaps and collide on next_id=37 vs my reserved F-037), carried the accurate forward backlog+progress from the unmerged #46 branch onto a fresh branch off origin/master (memory files only — the F-038 CODE change touches index.html.erb, independent of F-037's show.html.erb, so no stacking). That gave 8 ready items -> 1b gate CLOSED (>=5), correctly skipping new proposals. Phase 2 top unbuilt scorer = F-038 (2.55), not reserved by any open PR. Added a neutral BadgeComponent ("↩ <context_label>") to inbox cards when conversation.context?, distinct from the brand unread badge. Request spec asserts the label shows on a contextual thread and exactly one context badge renders across a contextual+plain pair. bin/verify green: 226 examples, 0 failures; packwerk+rubocop clean. Real screenshot (booted dev server, seeded contextual+plain inbox) confirms the badge on the marketplace-bike thread and its absence on the plain thread.
Learnings:
- Same-session back-to-back runs: master lags your OWN last unmerged PR. Branch off origin/master but restore backlog.json+progress.md from your prior unmerged branch (git checkout <branch> -- <files>) so memory is accurate and next_id doesn't collide — then only the current item's CODE differs from master (no PR stacking).

## 2026-08-11 17:22 — F-037
Outcome: shipped
PR: https://github.com/danecjensen/citysocial/pull/46
Changed: components/messaging/app/views/messaging/conversations/show.html.erb, spec/requests/messaging_spec.rb, routines/backlog.json, routines/progress.md
North Star: Completes the private-messaging do -> others-see -> react loop — the sender learns their neighbor actually saw the message.
Notes: Master GREEN on arrival (225 examples, packwerk+rubocop clean). 1a: DanesIdeas Inbox empty. Circuit breakers clear: only 4 open claude/* PRs (<6). All 4 master research briefs (R-005/006/008/009) are already built or in flight (merged #26/#30/#29, open #33), so none was a genuine unconsumed brief; grounded 1b in repo evidence instead. 1b gate open (4 ready<5): ran an Explore sweep of the 4 modules with ZERO open-PR file overlap (restaurants/events/feedback/messaging) and proposed F-037..F-041 from cited gaps. Top scorer F-037 (3.4): messaging read receipts — read_at is already persisted + loaded (set by mark_read_for! when the recipient opens the thread) but was only ever used for the unread badge count; the sender's own bubbles showed no "Read". Added a '✓ Read' on-token text label to own bubbles when read_at.present?, plus a request spec asserting it appears once the recipient opens the thread and is absent on received/unread messages. bin/verify green: 226 examples, 0 failures. Captured a real screenshot (booted dev server, seeded Ana<->Ben thread) confirming the read message shows the receipt and the unread one does not. Allocated F-037 (F-025..F-036 reserved by the open-PR queue / just-merged #45); next_id=42.
Learnings:
- Dead-end-loop vein pays out again (same shape as F-036): a module already persists AND loads a column but renders it in only one place (unread count) — surfacing it to the actor closes the loop with no migration.
- Folded the Playwright-login gotcha (ButtonComponent isn't type=submit; press Enter to submit) into the Codebase Patterns screenshot line.

## 2026-08-11 11:38 — F-036
Outcome: shipped
PR: https://github.com/danecjensen/citysocial/pull/45
Changed: components/restaurants/app/models/restaurants/vote.rb, components/restaurants/app/controllers/restaurants/leaderboard_controller.rb, components/restaurants/app/views/restaurants/leaderboard/index.html.erb, spec/models/restaurants/vote_spec.rb, spec/requests/restaurants_spec.rb, routines/backlog.json, routines/progress.md
North Star: Strengthens the do -> others-see loop by making community restaurant picks visible as social proof that invites more head-to-head voting.
Notes: Master GREEN on arrival (220 examples). 1a: DanesIdeas Inbox empty. No fresh unconsumed research briefs (R-005 -> open PR #33, R-006/R-008 already shipped via merged PRs #26/#30, status lines stale on master). 1b gate open (4 ready < 5, all below 1.0). Chose the strongest grounded, isolated, engagement-loop candidate from an Explore sweep of the four modules with NO open PR (restaurants/events/messaging/feedback): restaurant votes were persisted and even published as restaurants.matchup_decided but rendered in no view. Added Vote.recent (created_at-indexed), a batched recent-picks resolver in LeaderboardController (one query for all names, orphaned decisions skipped), and a Recent picks section on the leaderboard composing CardComponent + EmptyState. bin/verify green: 225 examples, 0 failures; packwerk + rubocop clean. Captured a real screenshot (booted dev server, login -> leaderboard, seeded 5 restaurants + 4 votes) showing the feed newest-first below the table. Allocated F-036 (not F-025) because F-025..F-035 are reserved by the concurrent open-PR queue (codex + claude tracks); set next_id=37.
Learnings:
