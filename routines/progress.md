...
## 2026-08-09 05:14 — F-026
Outcome: shipped
PR: https://github.com/danecjensen/citysocial/pull/34
Changed: components/pickup_sports/app/controllers/pickup_sports/games_controller.rb, components/pickup_sports/app/controllers/pickup_sports/roster_entries_controller.rb, components/pickup_sports/app/models/pickup_sports/game.rb, components/pickup_sports/app/models/pickup_sports/roster_entry.rb, components/pickup_sports/app/views/pickup_sports/games/show.html.erb, components/pickup_sports/config/routes.rb, components/pickup_sports/lib/pickup_sports/events.rb, docs/roadmap.md, spec/models/pickup_sports/game_spec.rb, spec/requests/pickup_sports_spec.rb, routines/backlog.json, routines/progress.md
Notes: Product lane. Shipped Pickup Sports milestone 2: after a game starts, only the host can mark rostered residents attended or absent; waitlisted residents stay excluded, every rostered player must be reviewed before closeout, completed games keep a public final roster, and the host can correct individual records afterward. Content-free attendance and closeout events preserve the module boundary and prepare later delivery without adding a Notifications dependency. F-025 remained reserved by open PR #33. Verification incomplete only because the managed sandbox denied PostgreSQL TCP before examples (focused 2 load errors/0 examples; full 44 load errors/0 examples). Packwerk, full RuboCop with a writable cache, Zeitwerk, Ruby syntax, Rails-aware ERB compilation, attendance routes, Tailwind, backlog integrity, diff whitespace, and exact remote feature-tree equality passed. A real Playwright screenshot required the unavailable database-backed authenticated page.
