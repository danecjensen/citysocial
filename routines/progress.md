...
- Git flow (default branch `master`): `git push` over Bash WORKS (probe `git push --dry-run`); prefer it over MCP `push_files` (hand-copying backlog.json risks JSON corruption). ALWAYS base a branch on `origin/master` after `git fetch origin master` — the fresh clone's local `master` ref can be a STALE old clone commit while HEAD is detached at the real remote master, so `git checkout -B <b> master` lands you on an ancient tree missing modules; use `git checkout -B <b> origin/master`. Then commit locally, `git push -u origin <branch>` (backoff on network errors), MCP `create_pull_request` (draft).
- `Notifications::DeliverActivity` is a data-driven follower fan-out: a new `<module>.<thing>_created` broadcast to an author's followers needs only (1) an `ACTIVITY` map entry `{source_key:, target_path:, action:}`, (2) a `Notifications::Events.subscriptions` line, and (3) the event added to `Notification::SUPPORTED_EVENTS` (a validated whitelist). `DeliverDirect` is the analogous path for single-recipient delivery (payload carries `recipient_id`).
...
## 2026-08-11 — F-035
Outcome: shipped
PR: https://github.com/danecjensen/citysocial/pull/43
Changed: components/notifications/app/models/notifications/notification.rb, components/notifications/app/services/notifications/deliver_activity.rb, components/notifications/lib/notifications/events.rb, spec/models/notifications/deliver_activity_spec.rb, spec/lib/notifications/events_spec.rb, routines/backlog.json, routines/progress.md
North Star: Completes the do->others-see->notified loop for the Craigslist-core marketplace by broadcasting a neighbor's new listing to their followers, reusing the proven follower-activity path.
Notes: Master green on arrival (bin/verify 220 examples). DanesIdeas Inbox empty (1a ingested nothing). 4 ready items all below the 1.0 threshold, so the 1b gate opened. GOTCHA hit and recovered: my first `git checkout -B ... master` resolved to a STALE local master ref (7423bdb, pre-modules) though the session's earlier work ran against detached HEAD at the real remote master b575592; re-fetched and rebased the branch onto origin/master before implementing. Picked the top grounded, unreserved gap: marketplace.listing_created was published but had no subscriber, unlike feed/communities which fan out via DeliverActivity. Score 3.6 (impact 4 x conf 0.9 / effort 1) beat the other open loop gaps (messaging DM, pickup roster promotion) because those need a new handler (effort 3, score ~1.3). Wired marketplace.listing_created through DeliverActivity (one ACTIVITY entry + one subscription + SUPPORTED_EVENTS entry). bin/verify GREEN on real PostgreSQL: 221 examples/0 failures (+1 marketplace fan-out spec), packwerk + rubocop clean. 1b seeded 4 grounded follow-ups from a scout sweep (F-036 DM notifications 1.33, F-037 pickup roster notifications 1.07, F-038 marketplace photos N+1 1.8, F-039 community comment-author N+1 0.85). Reserved IDs on open PRs (F-025..F-034) avoided by starting at F-035; next_id=40. Open claude/* PR count now 3 (#41,#42,#43) — under the 6-PR breaker.
Learnings:
- Promoted two Codebase Patterns: branch off origin/master (not the stale local master ref), and the DeliverActivity/DeliverDirect data-driven notification fan-out recipe.
