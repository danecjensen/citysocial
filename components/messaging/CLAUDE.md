...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/messaging/events.rb).
- Anything other modules need from here goes in `app/public/messaging/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `Messaging::ApplicationRecord` and use the table prefix `messaging_`.
