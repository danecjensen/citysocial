...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/developer/events.rb).
- Anything other modules need from here goes in `app/public/developer/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `Developer::ApplicationRecord` and use the table prefix `developer_`.
