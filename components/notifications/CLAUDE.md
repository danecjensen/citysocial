...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/notifications/events.rb).
- Anything other modules need from here goes in `app/public/notifications/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `Notifications::ApplicationRecord` and use the table prefix `notifications_`.
