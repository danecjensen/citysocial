...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/feedback/events.rb).
- Anything other modules need from here goes in `app/public/feedback/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `Feedback::ApplicationRecord` and use the table prefix `feedback_`.
