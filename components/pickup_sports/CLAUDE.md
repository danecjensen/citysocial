...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/pickup_sports/events.rb).
- Anything other modules need from here goes in `app/public/pickup_sports/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `PickupSports::ApplicationRecord` and use the table prefix `pickup_sports_`.
