...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/shared_calendar/events.rb).
- Anything other modules need from here goes in `app/public/shared_calendar/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `SharedCalendar::ApplicationRecord` and use the table prefix `shared_calendar_`.
