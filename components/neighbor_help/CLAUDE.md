...
- Cross-module communication goes through `PlatformCore::EventBus` (see lib/neighbor_help/events.rb).
- Anything other modules need from here goes in `app/public/neighbor_help/`.

## Conventions
- Controllers inherit from `PlatformCore::BaseController`.
- Models inherit from `NeighborHelp::ApplicationRecord` and use the table prefix `neighbor_help_`.
